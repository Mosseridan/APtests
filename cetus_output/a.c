...
void mat_mul_pointer_alias2(int n, int * * a, int * * b, int * * c)
{
	...
	int * * d = c;
	#pragma cetus private(i, j) 
	#pragma loop name mat_mul_pointer_alias2#0 
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<((1L+n)+((3L*n)*n)))) private(i, j)
	for (i=0; i<n; i ++ )
	{
		c[i][0]=0;
		#pragma cetus private(j) 
		#pragma loop name mat_mul_pointer_alias2#0#0 
		#pragma cetus parallel 
		#pragma omp parallel for if((10000<(-2L+(3L*n)))) private(j)
		for (j=1; j<n; j ++ )
		{
			c[i][j]=d[i][j-1];
		}
	...
	#pragma cetus private(i, j, k) 
	#pragma loop name mat_mul_pointer_alias2#1 
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<(((1L+(3L*n))+((3L*n)*n))+(((3L*n)*n)*n)))) private(i, j, k)
	for (i=0; i<n; i ++ )
	{
		#pragma cetus private(j, k) 
		#pragma loop name mat_mul_pointer_alias2#1#0 
		#pragma cetus parallel 
		#pragma omp parallel for if((10000<((1L+(3L*n))+((3L*n)*n)))) private(j, k)
		for (j=0; j<n; j ++ )
		{
			#pragma cetus private(k) 
			#pragma loop name mat_mul_pointer_alias2#1#0#0 
			#pragma cetus reduction(+: c[i][j]) 
			#pragma cetus parallel 
			#pragma omp parallel for if((10000<(1L+(3L*n)))) private(k) reduction(+: c[i][j])
			for (k=0; k<n; k ++ )
